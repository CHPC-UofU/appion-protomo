...
def orderImageList(frame_tiltdata, non_frame_tiltdata=None, frame_aligned="True", export=False):
	...
	for key in tiltkeys:
		...
		try:
			num_frames_list.append(len(imagedata['use frames']))
		except:
	...
	if not testing:
		if export == False:
			...
			#print ordered_imagelist
			#import sys
			#sys.exit()
			return tiltkeys,ordered_imagelist,dose_list,accumulated_dose_list,mrc_files,refimg,defocus_list,apDatabase.getPixelSize(imagedata),dose_imagedata['scope']['magnification'],num_frames_list
	else:
