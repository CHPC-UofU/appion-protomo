...
def printTips(tip_type):
	...
	choices.append("Info: Need help? Is something broken? Check the github website and the Google group: https://github.com/nysbc/appion-protomo   https://groups.google.com/forum/#!forum/appion-protomo")
	...
	choices.append("Info: Tilt-series alignment depends on the objects in the ice not moving during tilt-series collection. Beam-induced motion and ice doming are not correctable with Protomo - sub-tilt-series refinement and optional re-mapping back onto individual tilt images and producing a 3D vector field may be a way to correct for these motions. Beam-induced motion refers to the aggregate frame-to-frame motion for all tilt images (assuming a movie camera is used). Doming refers to the ice itself bending during collection, thus changing the 3D locations and orientations of objects in the ice (remember, Protomo aligns using a central portion of the 3D volume).")
	...
	choices.append("Tip: Each microscope + holder will have a unique tilt azimuth. To determine this value, collect a tilt-series of high SNR objects without ice contamination and run Appion-Protomo refinement for 50-100 iterations at binned by 8, followed by 5-10 iterations at binned by 4, 2, then 1. Make sure the thickness values are chosen appropriately. If the alignment converged (CCMS values and correction factors are all very low), then the tilt azimuth has likely been estimated properly. You may wish to check the opposite tilt azimuth (plus or minus 180 degrees) to see if it aligns better or worse. Use the resulting value as input into all tilt-series alignment for this microscope + holder combination by inputting the value into the tilt-series collection software tilt axis model (+-90  degrees possibly), or inputting it into the 'Override Tilt Azimuth' parameter in Appion-Protomo. Collecting and processing a medium-magnification tilt-series may be the easiest method for determining tilt azimuth, assuming that the medium and high magnifications are aligned to each other on the microscope.")
	...
	choices.append("Tip: Batch Align Tilt-Series is particularly useful for initial tilt-series screening. After a large tilt-series collection, run one tilt-series through the Align Tilt-Series Coarse Alignment, then use the resulting coarse_series####.param file as input into Batch Coarse Alignment. Then on the Alignment Summary webpage, you can easily browse through each tilt-series to identify which are ready for Refinement.")
	choices.append("Tip: Refinement of most samples works best with the 10nm object(s) - Steep lowpass Preset Parameter. Don't forget to adjust the Thickness values appropriately after changing the presets!")
	...
	choices.append("Tip: The Scaled Sum CCMS value should be used as a guide to help determine a small set of well-aligned iterations. The best iteration depends on the purpose of the resulting tomogram. If all tilt angles are desired, for segmentation & visualization purposes for instance, then the best CCMS value at bin 2 or 1 is likely the best iteration. If the tomogram is intended for SPT of symmetric and/or non-preferred objects, then the set of good iterations should be individually checked to identify a well-aligned tilt range.")
	...
	choices.append("Tip: Tilt-series collected with SerialEM can be easily uploaded to Appion for alignment with Appion-Protomo by following the instructions on the github website: https://github.com/nysbc/appion-protomo")
	choices.append("Tip: Tilt-series collected not using Leginon or SerialEM can be imported using 'upload images' links found in the Project DB or on the left hand side of the Appion processing menu for a tilt-series session.")
	...
	choices.append("Tip: For running on clusters, consider installing Screen on a stable login node. This will allow you to run Appion-Protomo alignments in convenient virtual terminals without fear of losing your connection or accidentally closing your terminal window. See here for more info: https://www.tecmint.com/screen-command-examples-to-manage-linux-terminals")
	...
	if tip_type == "Alignment": #Alignment tips
		choices.append("Info: Protomo alignment assumes that the angles between tilt images are fixed; ie. tilt angles are never refined. These are the only values taht are not refined in Protomo.")
		...
		choices.append("Tip: You can restart an alignment using results from a previous alignment by inputting appropriate values into the Protomo Refinement webpage with a new Run name and/or Output directory.")
		choices.append("Tip: The easiest way to restart a refinement if by manually aligning the iteration that you want to restart from (you don't have to re-align anything if you don't need, just close the manual aligner), then go to the refinement page with the tilt-series run name that you manually aligned, and select 'Manual' under the 'Starting Alignment' Parameter Preset.")
		choices.append("Tip: CTF correction reduces the contrast of each image and thus may reduce the alignability of the tilt images. For this reason it is usually recommended that CTF correction not be performed before alignment.")
		choices.append("Tip: Dose compansation increases the contrast of most tilt images. For this reason it is recommended that dose compensation be performed before alignment for thin and/or dispersed samples. Dose compensation shouldn't be applied if doing so results in Protomo WBP reconstructions that look dark and washed out.")
		choices.append("Tip: Dose compensation is based on a fit to experimental data of proteins in cryo from Grant and Grigorieff, 2015 (Moderate uses values from the paper). As a result, dose compensation with Moderate may not be accurate for your sample. The amount of dose compensation can be adjusted as you determine is appropriate with the values a, b, and c.")
		choices.append("Tip: The alignment thickness is the estimated height in the z-direction of the objects of interest in the tomogram. This is a critical parameter and should be found and potentially optimized for each tilt-series.")
		choices.append("Tip: The correlation peak video should ideally have a bright, round dot in the center of each frame, indicating that there is signal for alignment and that the alignment is potentially good.")
		choices.append("Tip: A tilt-series that has an asymmetric positive-to-negative tilt range, e.g. [-45:60], will likely refine with a slightly different tilt azimuth compared to a symmetric tilt-series. For extreme cases such as a half tilt-series, e.g. [0:60], the tilt azimuth will very likely refine incorrectly. For either of these cases you may wish to assign the known tilt azimuth for the microscope+holder combination and turn off tilt azimuth refinement (Refinement Advanced settings).")
		choices.append("Tip: If there are grossly misaligned tilt images after Coarse Alignment that you wish to recover rather than discard, run the protomo2manualaligner.py command at the bottom of the summary webpage. Several options are available to assist with manual alignment.")
		#choices.append("Tip: Consider turning on the optional Center of Mass Peak Search option during Refinement. By turning on this option, Protomo will identify the 'center of mass' of intensity values in the correlation peak by searching in an ellipse centered on the highest intensity pixel found during peak search. This allows for sub-pixel peak precision and may increase or decrease the accuracy of an alignment.")
		...
		choices.append("Tip: You can make a reconstruction while a tilt-series is still aligning. Be careful with dose compensating during alignment though!")
		...
		choices.append("Tip: Reconstruction videos on alignment summary webpages include all grid refinement angles (sample orientation, tilt azimuth, and tilt elevation). For this reason, reconstructions with methods other than Protomo WBP will almost certainly be oriented differently from the Protomo WBP.")
		...
		choices.append("Tip: The reconstruction thickness is independent from the alignment thickness. The reconstruction thickness defines the thickness of the tomogram. A good practice is to first reconstruct at binned by 8 and with a large thickness (maybe 4000), then reconstruct again with a decreased thickness based on the locations of the objects of interest.")
...
def printCitations():
	...
	apDisplay.printMsg("doi:10.7554/eLife.34257")
	print ''
	apDisplay.printMsg("Reducing effects of particle adsorption to the air-water interface in cryoEM")
	apDisplay.printMsg("Noble, A. J., Wei, H., Dandey, V. P., Zhang Z., Tan Y. Z., Potter C. S., Carragher, B.")
	apDisplay.printMsg("https://doi.org/10.1038/s41592-018-0139-3")
	print '\n________________________________\n'
...
def stack(arrays, axis):
	"""
	Used by protomoToImodAndEman below, but requires a newer version of Numpy.
	This is the same function from the newer Numpy version.
	"""
...
def protomoToImodAndEman(tiltfilename, outdir, basename, pixelsize, dimx, dimy, tilt_azimuth, num_images=0, no_transform=False):
	"""
	Converts a Protomo tlt file to an IMOD xf file and an EMAN2 json file.
	If no_transform=True, it will just create an identity xf file.
	Created with help/input from: David Mastronarde, Sagar Khavnekar, Sargis Dallakyan, Alister Burt, Muyuan Chen, Jake Johnston, and Reza Paraan.
	"""
	#-----------------DEFINITIONS-----------------#
	#Each linear transformation in a transform file is specified by a line with six numbers:
	#  A11 A12 A21 A22 DX DY
	#where the coordinate (X, Y) is transformed to (X', Y') by:
	#  X' = A11 * (X - Xci) + A12 * (Y - Yci) + DX + Xco
	#  Y' = A21 * (X - Xci) + A22 * (Y - Yci) + DY + Yco
	# |X'|_|A11 A12| |X - Xci| + DX + Xco
	# |Y'|-|A21 A22|*|Y - Yci| + DY + Yco
	#where (Xci, Yci) and (Xco, Yco) are the center coordinates of the input and output images, respectively.
	#Going from an isotropic scaling to the xf format should be no problem. You just multiply your rotation matrix by
	# | s 0 |
	# | 0 s |
	# |X'|_|s 0| |A11 A12| |X - Xci| + DX + Xco
	# |Y'|-|0 s|*|A21 A22|*|Y - Yci| + DY + Yco
	#Note: Scaling is not included in the converted xf & json transforms below because I don't know if/which post-processing software deals with scaling properly.
	#But I may include it for .xf because it should just be a multiplicative factor as described above...
	
	#if no_transform: #Outputs an xf file without any transformations - ie. no shift, no rotation, no scale, no skew; ie. identity matrix
			#f.write('  1.0000000   0.0000000   0.0000000   1.0000000       0.000       0.000\n')
		#f.close()
		
		#return

	#Convert to xf and json
	lines = open(tiltfilename).readlines()
	start_number = None
	end_number = 100000
	flag_found_images = False
	for line in lines: #need this to get to start and end numbers in IMAGES
		if line.startswith("   IMAGE"):
			flag_found_images = True
			if start_number is None:
				start_number = int(line.split("IMAGE")[1].split()[0])
			else:
				number = int(line.split("IMAGE")[1].split()[0])
		elif flag_found_images:
			end_number = number
	...
	outxf = "%s.xf" % basename
	outjson = "info/%s_info.json" % basename
	outrawtlt = "%s.mrc.tlt" % basename
	os.system('mkdir -p info tiltseries 2>/dev/null')
	open(outjson,'w').write("{\n")
	open(outjson,'a').write('"apix_unbin": ' + str(pixelsize) + ",\n")
	open(outjson,'a').write('"tlt_file": ' + '"tiltseries/' + basename + '.mrc"' + ",\n")
	open(outjson,'a').write('"tlt_params": ' + "[")

	# Convert Protomo tlt to EMAN2 json
	files=''
	files_list_protomo=[]
	#tlt_angles_protomo=[]
	for i in range(start_number,end_number+1):
		# Get information from tlt file. This needs to be versatile for differently formatted .tlt files, so awk it is.
		try: #breaks if image wasn't included in the requested Protomo .tlt file
			cmd1 = "awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/FILE/) print $(j+1)}' | tr '\n' ' ' | sed 's/ //g'" % (i, tiltfilename)
			proc = subprocess.Popen(cmd1, stdout=subprocess.PIPE, shell=True, universal_newlines=True)
			(filename, err) = proc.communicate()
			cmd2 = "awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/ORIGIN/) print $(j+2)}'" % (i, tiltfilename)
			proc = subprocess.Popen(cmd2, stdout=subprocess.PIPE, shell=True, universal_newlines=True)
			(originx, err) = proc.communicate()
			originx = float(originx)
			cmd3 = "awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/ORIGIN/) print $(j+3)}'" % (i, tiltfilename)
			proc = subprocess.Popen(cmd3, stdout=subprocess.PIPE, shell=True, universal_newlines=True)
			(originy, err) = proc.communicate()
			originy = float(originy)
			cmd4 = "awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/ROTATION/) print $(j+1)}'" % (i, tiltfilename)
			proc = subprocess.Popen(cmd4, stdout=subprocess.PIPE, shell=True, universal_newlines=True)
			(rotation, err) = proc.communicate()
			rotation = float(rotation)
			cmd5 = "awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/ANGLE/) print $(j+1)}'" % (i, tiltfilename)
			proc = subprocess.Popen(cmd5, stdout=subprocess.PIPE, shell=True, universal_newlines=True)
			(tilt_angle, err) = proc.communicate()
			tilt_angle = float(tilt_angle)
			
			# (cin - cref) and (90 - rotation) seems to be correct for protomo_to_eman2
			cin = np.array((originx, originy))
			center = (int(dimx/2), int(dimy/2))
			cref = np.array(center)
			shift = cin - cref
			eman2_transform = "[" + str(shift[0]) + "," + str(shift[1]) + "," + str(90-rotation) + "," + str(tilt_angle) + "," + str(tilt_azimuth) + "]"
			if i != end_number:
				eman2_transform += ","
			open(outjson,'a').write(eman2_transform + "\n")
			open(outrawtlt,'a').write(str(tilt_angle) + "\n")
			files+=' raw/%s.mrc' % filename
			files_list_protomo.append(filename)
			#tlt_angles_protomo.append(tilt_angle)
		except: #image was removed
			pass

	open(outjson,'a').write("]\n")
	open(outjson,'a').write("}\n")
	apDisplay.printMsg("The Protomo .tlt file has been converted to an EMAN2 .json file.")

	# Convert EMAN2 json to IMOD xf
	# Code adopted and modified from https://github.com/alisterburt/emanjson2imodxf/tree/master/emanjson2imodxf
	with open(outjson) as f:
		data = json.load(f)
	tlt_params = np.array(data['tlt_params'])
	dx = tlt_params[:, 0]
	dy = tlt_params[:, 1]
	dxy_eman = -stack((dx, dy), axis=-1).reshape((-1, 2, 1))
	theta = np.deg2rad(tlt_params[:, 2] + tlt_params[:, 4])
	rotation_matrices = np.array([[np.cos(theta), -np.sin(theta)],[np.sin(theta), np.cos(theta)]])
	rotation_matrices = rotation_matrices.swapaxes(0, 2)
	dxy_imod = np.dot(rotation_matrices,dxy_eman)
	...
	for i in range(len(A11)):
		#f.write('  %.6f   %.6f   %.6f   %.6f       %.6f       %.6f\n' % (A11[i], A12[i], A21[i], A22[i], dx_imod[0][i], dy_imod[0][i]))
		f.write('%.6f %.6f %.6f %.6f %.6f %.6f\n' % (A11[i], A12[i], A21[i], A22[i], dx_imod[0][i], dy_imod[0][i]))
	f.close()

	apDisplay.printMsg("The Protomo .tlt file has been converted to an IMOD .xf file.")

	cmd7 = 'newstack %s -on tiltseries/%s.mrc'%(files, basename)
	proc = subprocess.Popen(cmd7, stdout=subprocess.PIPE, shell=True, universal_newlines=True)
	out = proc.communicate()
	apDisplay.printMsg("The unaligned tilt-series stack has been created.")
	apDisplay.printMsg("The rawtlt angle file has been created.")
	
	return files_list_protomo
...
def hyphen_range(s):
	...
	for x in s.split(','):
	    t=x.split('-')
	    if len(t) not in [1,2]: raise SyntaxError("hash_range is given its argument as "+s+" which seems not correctly formated.")
...
def centerAlignment(tiltfilename_full, dim_x, dim_y):
	...
	The search area during Protomo refinement emanates from the center of the alignment. This function maximizes that in-common area by shifting all tilt images appropriately.
	'''
	...
	max_origin_x_pos = -999999
	max_origin_x_neg = 999999
	max_origin_y_pos = -999999
	max_origin_y_neg = 999999
	for i in range(tiltstart-1,tiltstart+numimages+100):
		try:
			...
			max_origin_x_pos = max(max_origin_x_pos, originx)
			max_origin_x_neg = min(max_origin_x_neg, originx)
			max_origin_y_pos = max(max_origin_y_pos, originy)
			max_origin_y_neg = min(max_origin_y_neg, originy)
		except:
			pass
	
	avg_origin_x = (max_origin_x_pos + max_origin_x_neg)/2
	avg_origin_y = (max_origin_y_pos + max_origin_y_neg)/2
	distance_x = (dim_x/2) - avg_origin_x
	distance_y = (dim_y/2) - avg_origin_y
		...
		if ('IMAGE' in line) and ('ORIGIN' in line):
			...
			for i in range(len(strings)):
				if strings[i] == 'ORIGIN':
					...
					new_x_num = old_x_num + distance_x
					new_y_num = old_y_num + distance_y
					new_x = '%.3f' % new_x_num
					new_y = '%.3f' % new_y_num
	...
	apDisplay.printMsg("Images were re-centered by a distance of: (x, y) = (%.3f, %.3f)" % (distance_x, distance_y))
	
	return


def shiftAlignment(tiltfilename_full, dim_x, dim_y, shift_x, shift_y):
	'''
	This will shift all images in the provided Protomo .tlt file with respect to the center of the alignment.
	The search area during Protomo refinement emanates from the center of the alignment. This function first maximizes that in-common area by shifting all tilt images appropriately, then applies a user-defined (x,y) shift on top of that.
	'''
	apDisplay.printMsg("Shifting alignment...")
	cmd1="awk '/ORIGIN /{print}' %s | wc -l" % (tiltfilename_full)
	proc=subprocess.Popen(cmd1, stdout=subprocess.PIPE, shell=True)
	(numimages, err) = proc.communicate()
	numimages=int(numimages)
	...
	avg_origin_x = (max_origin_x_pos + max_origin_x_neg)/2
	avg_origin_y = (max_origin_y_pos + max_origin_y_neg)/2
	distance_x = (dim_x/2) - avg_origin_x - shift_x
	distance_y = (dim_y/2) - avg_origin_y - shift_y
	...
	apDisplay.printMsg("Images were shifted from the alignment center by a distance of: (x, y) = (%.3f, %.3f)" % (distance_x, distance_y))
	
	return


def shiftAlignmentByCorrs(tiltfilename_full, dim_x, dim_y, corrfile, scale):
	'''
	This will shift and optionally rotate images (no scaling) in the provided Protomo .tlt file by the correction factors, scaled by a user-defined amount.
	'''
	apDisplay.printMsg("Shifting/rotating alignment using correction factors...")
	cmd1="awk '/ORIGIN /{print}' %s | wc -l" % (tiltfilename_full)
	proc=subprocess.Popen(cmd1, stdout=subprocess.PIPE, shell=True)
	(numimages, err) = proc.communicate()
	numimages=int(numimages)
	...
	for i in range(tiltstart-1,tiltstart+numimages+100):
		try:
			cmd1="awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/ORIGIN/) print $(j+2)}'" % (i, tiltfilename_full)
			proc=subprocess.Popen(cmd1, stdout=subprocess.PIPE, shell=True)
			(originx, err) = proc.communicate()
			originx=float(originx)
			cmd2="awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/ORIGIN/) print $(j+3)}'" % (i, tiltfilename_full)
			proc=subprocess.Popen(cmd2, stdout=subprocess.PIPE, shell=True)
			(originy, err) = proc.communicate()
			originy=float(originy)
			cmd3="awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/ROTATION/) print $(j+1)}'" % (i, tiltfilename_full)
			proc=subprocess.Popen(cmd3, stdout=subprocess.PIPE, shell=True)
			(rotation, err) = proc.communicate()
			rotation=float(rotation)
			cmd4="awk '/REFERENCE /{print}' %s | awk '{for (j=1;j<=NF;j++) print $(j+2)}'" % (tiltfilename_full)
			proc=subprocess.Popen(cmd4, stdout=subprocess.PIPE, shell=True)
			(refimg, err) = proc.communicate()
			refimg=int(refimg)
			refimg_name = 'IMAGE %s' % refimg
			
			new_origins_x.append(originx + (abs(1-cofx[j])*dim_x*scale))
			new_origins_y.append(originy + (abs(1-cofy[j])*dim_y*scale))
			new_rotations.append(rotation + (rot[j]*scale))
			j=j+1
		except:
			pass
	...
	j=0
	for line in lines:
		if ('IMAGE' in line) and ('ORIGIN' in line) and (refimg_name not in line):
			strings=line.split()
			old_x=strings[4]
			old_y=strings[5]
			old_x_num = float(old_x)
			old_y_num = float(old_y)
			new_x_num = new_origins_x[j]
			new_y_num = new_origins_y[j]
			new_x = '%.3f' % new_x_num
			new_y = '%.3f' % new_y_num
			f.write(line.replace(old_x,new_x).replace(old_y,new_y))
			j=j+1
	...
	j=0
	for line in lines:
		if ('IMAGE' in line) and ('ROTATION' in line) and (refimg_name not in line):
			strings=line.split()
			print strings
			old_rot=strings[6]
			old_rot_num = float(old_rot)
			new_rot_num = new_rotations[j]
			new_rot = '%.3f' % new_rot_num
			f.write(line.replace(old_rot,new_rot))
			j=j+1
		else:
			f.write(line)
	f.close()
	
	apDisplay.printMsg("Images were shifted/rotated based on the correction factors")
...
def changeReferenceImage(tiltfilename_full, desired_ref_tilt_angle, msg=True):
	'''
...
def reOrderImagesInTltFile(tiltfilename_full):
	'''
	This just re-orders the tilt images in the Protomo tlt file so that they go from negative to positive.
	This is useful because it effects the order of images in tilt-series videos and in the manual aligner.
	This function was written to work on initial tlt files that have all of each images' information in one line.
	'''
	cmd1="awk '/ORIGIN /{print}' %s | wc -l" % (tiltfilename_full)
	proc=subprocess.Popen(cmd1, stdout=subprocess.PIPE, shell=True)
	(numimages, err) = proc.communicate()
	...
	(tiltstart, err) = proc.communicate()
	tiltstart=int(tiltstart)
	cmd3="awk '/REFERENCE IMAGE /{print $3}' %s | head -n +1" % (tiltfilename_full)
	proc=subprocess.Popen(cmd3, stdout=subprocess.PIPE, shell=True)
	(refimg, err) = proc.communicate()
	refimg=int(refimg)
	
	full_list=[]
	for i in range(0,tiltstart+numimages+100):
		try: #If the image isn't in the .tlt file, skip it
			cmd1="awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/TILT/) print $(j+2)}'" % (i+1, tiltfilename_full)
			proc=subprocess.Popen(cmd1, stdout=subprocess.PIPE, shell=True)
			(tilt_angle, err) = proc.communicate()
			tilt_angle=float(tilt_angle)
			cmd2="awk '/IMAGE %s /' %s" % (i+1, tiltfilename_full)
			proc=subprocess.Popen(cmd2, stdout=subprocess.PIPE, shell=True)
			(line, err) = proc.communicate()
			line=str(line)
			line_listed = {'tilt_angle':tilt_angle,'line':line}
			full_list.append(line_listed)
		except:
			pass
	
	new_full_list = sorted(full_list, key=lambda k: k['tilt_angle'])
	
	tiltfilename_full_tmp = tiltfilename_full + '_tmp'
	f = open(tiltfilename_full_tmp, 'w')
	with open(tiltfilename_full) as tltfile:
		top = [next(tltfile) for x in xrange(7)]
	tltfile.close()
	...
	for i in range(0,tiltstart+numimages+100):
		try: #Probably don't need this try
			line = new_full_list[i]['line']
			replace_this = 'IMAGE ' + line.split()[1]
			with_this = 'IMAGE %d' % (i+1)
			f.write(line.replace(replace_this, with_this))
		except:
			pass
	
	f.write('\n   REFERENCE IMAGE %d' % refimg); f.write('\n\n\n END')
	f.close()
	
	changeReferenceImage(tiltfilename_full_tmp, 0, msg=False)
	
	os.system('mv %s %s' % (tiltfilename_full_tmp, tiltfilename_full))
...
def removeImageFromMdocFile(tiltfile, imagenumber):
	'''
	This removes all entries 'IMAGE $imagenumber' from a .tlt file. No backups made.
	'''
	#First check to make sure that the reference image is not being asked to be removed
...
def makeCorrPlotImages(seriesname, iteration, rundir, corrfile):
	...
	try: #If anything fails, it's likely that something isn't in the path
		...
		mintilt=0
		maxtilt=0
		for i in range(tiltstart-1,tiltstart+rawimagecount+100):
			try:
...
def makeTiltSeriesVideos(seriesname, iteration, tiltfilename, rawimagecount, rundir, raw_path, pixelsize, map_sampling, image_file_type, video_type, tilt_clip, parallel, align_step):
	...
	def processTiltImages(i,j,tiltfilename,raw_path,image_file_type,map_sampling,rundir,pixelsize,rawimagecount,tilt_clip):
		try: #If the image isn't in the .tlt file, skip it
			...
			if pixelsize == 0:
				command3 = "convert -gravity South -background white -splice 0x16 -annotate 0 'Tilt Image: %s/%s' -gravity SouthEast -annotate 0 '%s %s' %s %s" % (j+1, rawimagecount, tilt_degrees, degrees, tiltimage, tiltimage)
			else:
	...
	try: #If anything fails, it's likely that something isn't in the path
		...
		# el
		if (parallel=="True") and (mp.cpu_count()-1 > 2):
		...
		if align_step == "Initial":
			...
			apDisplay.printMsg("Done creating Protomo coarse tilt-series video!")
		elif align_step == "Coarse2":
			apDisplay.printMsg("Done creating Protomo coarse iteration 2 tilt-series video!")
		elif align_step == "Imod":
...
def makeReconstructionVideos(seriesname, iteration, rundir, rx, ry, show_window_size, outdir, pixelsize, sampling, map_sampling, lowpass, thickness, video_type, keep_recons, parallel, align_step):
	...
	def processReconImages(i,slices,vid_path,volume,minval,maxval,pixelsize,map_sampling,dimx,dimy,show_window_size,rx,ry):
		...
		#Add frame numbers
		if pixelsize == 0:
			cmd = "convert -gravity South -background white -splice 0x16 -annotate 0 'Z-Slice: %s/%s' -gravity SouthEast -annotate 0 'bin=%s' %s %s" % (i+1, slices+1, map_sampling, slice, slice)
		else:
	...
	try: #If anything fails, it's likely that something isn't in the path
		...
		if keep_recons == "false":
			...
			os.system(command3)
		apDisplay.printMsg("Done creating Protomo reconstruction video!")
...
def makeDriftPlot(rundir, seriesname, tilts, ordered_mrc_files, pixelsize):
	'''
	Creates a plot of the maximum tilt frame image drift versus tilt angle.
	'''
