...
#=======================
def cleanUpFilename(full_file_path):
	'''
	Replaces the following bad characters in a filename with an underscore (_):
	,, ., [, ], {, }, (, )
	'''
	full_path = os.path.dirname(full_file_path)
	filename = os.path.basename(full_file_path)
	name = os.path.splitext(filename)[0]
	ext = os.path.splitext(filename)[1]
	new_name = name.replace('.','_').replace(',','_').replace('[','_').replace(']','_').replace('{','_').replace('}','_').replace('(','_').replace(')','_')
	new_full_file_path = os.path.join(full_path, new_name + ext)
	if (full_file_path != new_full_file_path):
		apDisplay.printMsg("The filename has been changed to %s to avoid naming conflicts." % os.path.basename(new_full_file_path))
		os.system('mv %s %s' % (full_file_path, new_full_file_path))
...
def prepareTiltFile(sessionname, seriesname, tiltfilename, tiltseriesnumber, raw_path, frame_aligned_images, link=False, coarse=True, export=False):
	...
	try:
		if export == False:
			...
			tilts,ordered_imagelist,dose_list,accumulated_dose_list,ordered_mrc_files,refimg,defocus_list,pixelsize,magnification,num_frames_list = apTomo.orderImageList(frame_tiltdata, non_frame_tiltdata, frame_aligned=frame_aligned_images, export=True)
	except TypeError:
	...
	if frame_aligned_images == "True":  #Azimuth is only present in the non-frame aligned images
		a,ordered_imagelist_for_azimuth,c,original_ordered_mrc_files,e,f = apTomo.orderImageList(frame_tiltdata, non_frame_tiltdata, frame_aligned="False")
	...
	if coarse == "True":
		...
		if export == False:
			...
			newfilenames, new_ordered_imagelist = apProTomo.getImageFiles(ordered_imagelist, raw_path, link=False, copy="True", pixelsize=pixelsize)
			...
			try:
				...
				imagesizex = len(mrc.read(new_ordered_imagelist[0])[0])
				imagesizey = len(mrc.read(new_ordered_imagelist[0]))
			...
			return tilts, dose_list, accumulated_dose_list, new_ordered_imagelist, maxtilt, original_filename_split[0], defocus_list, pixelsize, magnification, original_ordered_mrc_files, num_frames_list
...
def addOrientationAngles(tiltfilename_full, psi, theta, phi):
		"""
		Inserts three lines into a Protomo .tlt file containing Psi, Theta, and Phi.
		"""
		command1="grep -n 'TILT AZIMUTH' %s | awk '{print $1}' | sed 's/://'" % (tiltfilename_full)
		proc=subprocess.Popen(command1, stdout=subprocess.PIPE, shell=True)
		(azimuthline, err) = proc.communicate()
		insertline=int(azimuthline)+1
		
		with open(tiltfilename_full, "r") as f:
			contents = f.readlines()

		contents.insert(insertline, '\n')
		contents.insert(insertline+1, '   ORIENTATION\n')
		contents.insert(insertline+2, '\n')
		contents.insert(insertline+3, '     PSI    %s\n' % psi)
		contents.insert(insertline+4, '     THETA    %s\n' % theta)
		contents.insert(insertline+5, '     PHI    %s\n' % phi)
		contents.insert(insertline+6, '\n')

		with open(tiltfilename_full, "w") as f:
...
def rotateAndTranslateImage(i, tiltfilename, rundir, recon_dir, mrc_list, tilt_list):
		"""
		Rotates and translates a single image from Protomo orientation to IMOD orientation.
		"""
		try:
...
def tomo4toAppion(stack, txt, rawtlt, voltage, dose):
	...
	while current_tilt <= max_tilt:
		...
		tilt_info = '%s\t%fe-10\t%d\t%d\t%d\t%fe-6\t%d\t%f\t%f\n' % (filename, pixelsize, binning, binning, mag, defocus, int(voltage)*1000, current_tilt, dose)
