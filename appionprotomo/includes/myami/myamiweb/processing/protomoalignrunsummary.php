...
$all_unbinned_stacks = array();   // initialization
...
$html .= "<br><br><b>To continue aligning a tilt-series, first manually align it, then <a href='runAppionLoop.php?expId=$expId&form=Protomo2CoarseAlignForm' target='_blank'>Click Here</a>, select the appropriate tilt-series, and change the runname (e.g. tiltseries0004_thick500) & output directory, then continue to Refinement.</b><br>";
...
foreach($tiltseries_runs as $tiltseries_run) {
	...
	if ($protomo2aligner_logs[0] != '') {
		$html .= "<td><center><b>$path_chunks[1]</b><br>$date_m[0]-$date_d[0]-$date_yr[0]<br>@ $time_hr[0]:$time_m[0]<br><input type='checkbox' onclick='return writeTo(this)' name='check_list' value='$outdir/$path_chunks[1]/'><sup>remove? </sup><input type='checkbox' onclick='return writeTo2(this)' name='check_list' value='$path_chunks[1]'><sup>list</sup></center>";
	}else{
		$html .= "<td><center><b>$path_chunks[1]</b><br><input type='checkbox' onclick='return writeTo(this)' name='check_list' value='$outdir/$path_chunks[1]/'><sup>remove? </sup><input type='checkbox' onclick='return writeTo2(this)' name='check_list' value='$path_chunks[1]'><sup>list</sup></center>";
	}
	$html .= '<script>
	...
	$html .= '<script>
				function writeTo2(object) {
					  var container2 = document.getElementById("container2");
					  if (object.checked) {
						 container2.innerHTML = container2.innerHTML + object.value + ",";   
					  }
					}
				</script>';
	...
	$unbinned_stacks = glob("$outdir/$path_chunks[1]/stack/*bin1*.mrcs",GLOB_BRACE);  //Might as well count unbinned stacks while we're here
	$all_unbinned_stacks = array_merge($all_unbinned_stacks,$unbinned_stacks);
...
$html .= "</table>";
$html .= "<br>";
$html .= "<b>To continue aligning a tilt-series, first manually align it, then <a href='runAppionLoop.php?expId=$expId&form=Protomo2CoarseAlignForm' target='_blank'>Click Here</a>, select the appropriate tilt-series, and change the runname (e.g. tiltseries0004_thick500) & output directory, then continue to Refinement.</b><br><br>";
//$html .= "<b>Estimate defocus or dose compensate existing tilt-series by continuing to Batch or to More Tilt-Series Processing.</b>";
...
if (count($all_unbinned_stacks) > 0) {
	$html .= "
	<br><hr />
	<H4><b>Available Unbinned Aligned Tilt-series Stacks</b></H4>
	<hr />";
	
	foreach ($all_unbinned_stacks as $item) {
		$html .= '<br>'.$item;
	}
	$html .= '<br>';
}

$html .= "<br><hr /><H4><b>List of good tilt-series runs that you want to batch export for high-res processing (unchecking boxes doesn't work, sorry!):</b></H4><hr /><br>";
