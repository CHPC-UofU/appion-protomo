...
class ProTomo2Reconstruction(basicScript.BasicScript):
	...
	def setupParserOptions(self):
		...
		self.parser.add_option("--pixelsize_multiplier", dest="pixelsize_multiplier",  default="1",
		...
		self.parser.add_option("--reconstruction_actions", dest="reconstruction_actions",  type="int",
			help="Actions and order of actions to perform. 1: Reconstruct, 2: CTF correct then reconstruct, 3: Dose compensate then reconstruct, 4: CTF correct then dose compensate then reconstruct, 5: Export for post-processing e.g. --reconstruction_actions=2", metavar="int")
		...
		self.parser.add_option("--rot90", dest="rot90", type="int", default=0,
			help="Number of times to rotate images 90 degrees (hidden option in case it doesn't rotate properly for some reason), e.g. --rot90=3", metavar="int")
			
		self.parser.add_option("--flip_newshape", dest="flip_newshape", type="int", default=0,
			help="Flip new shape of tilt-series (hidden option in case it doesn't flip properly for some reason), e.g. --flip_newshape=1", metavar="int")
	...
	def rotateAndTranslateAndMaybeScaleImage2(self, i, tiltfilename, rundir, recon_dir, tilt_list, azimuth, newshape, rot90):
		"""
		Rotates and tanslates a single image from Protomo orientation to IMOD orientation.
		Scales images if .tlt file specifies scaling.
		Uses a single 5th order interpolation for the entire affine transformation.
		"""
		try:
			#Get information from tlt file. This needs to versatile for differently formatted .tlt files, so awk it is.
			cmd1="awk '/IMAGE %s /{print}' %s | awk '{for (j=1;j<=NF;j++) if($j ~/FILE/) print $(j+1)}' | tr '\n' ' ' | sed 's/ //g'" % (i, tiltfilename)
			...
			mrcf_out=os.path.join(recon_dir,'tomo3d',filename+'.mrc')
			im=mrc.read(mrcf)
			rot = 90 + azimuth + rotation + 90*rot90
			an=np.deg2rad(rot)
	...
	def start(self):
		...
		if (self.params['reconstruction_actions'] != 5):
		...
		if (self.params['reconstruction_actions'] != 5):
			...
			if self.params['reconstruction_method'] == 1:
				...
				# Normalize
				try:
					command="newstack -q -meansd 0,1 %s %s" % (mrc_full, mrcn_full)
			...
			# Stack and Tomo3D Setup
				...
				if (self.params['reconstruction_actions'] == 1 or self.params['reconstruction_actions'] == 3):
					...
					mrc_list=[]
					tilt_list=[]
					newlist = [f for f in os.listdir(raw_path) if f.endswith('.mrc')]
					dimy,dimx = mrc.read(os.path.join(raw_path,newlist[0])).shape
					# dimx=0
					# dimy=0
					...
					try:
						...
						pass
					if self.params['flip_newshape'] == 1:
						newshape=(newshape_y, newshape_x)
					else:
						newshape=(newshape_x, newshape_y)
					for i, j in zip(range(tiltstart,numimages+tiltstart+202), range(numimages+201)):
						p = mp.Process(target=self.rotateAndTranslateAndMaybeScaleImage2, args=(i, recon_tilt_out_full, self.params['rundir'], recon_dir, tilt_list, azimuth, newshape, self.params['rot90']))
						p.start()
					...
					#if self.params['rot90'] != 0: # Need to un-rotate tilt-series for reconstructing with y-axis=tilt-axis
					#	stack=np.rot90(stack,k=-self.params['rot90'],axes=(1,0))
					
					mrc.write(stack,stack_path)
					apixdict = {'x':self.params['pixelsize'], 'y':self.params['pixelsize'], 'z':self.params['pixelsize']}
					mrc.updateFilePixelSize(stack_path, apixdict)
				...
				if self.params['recon_map_sampling'] > 1:
					...
					binned_pixelsize=self.params['pixelsize']*self.params['recon_map_sampling']
					apixdict = {'x':binned_pixelsize, 'y':binned_pixelsize, 'z':binned_pixelsize}
					mrc.updateFilePixelSize(stack_path, apixdict)
			...
			if self.params['reconstruction_method'] == 2 or self.params['reconstruction_method'] == 3:
				...
				print out, err
				apDisplay.printMsg("Rotating reconstruction...")
				try:
					os.system('trimvol -rx %s %s' % (mrc_full, mrc_full))
					os.system('rm %s~' % mrc_full)
					apDisplay.printMsg("Normalizing reconstruction...")
					cmd = 'newstack -q -meansd 0,1 %s %s' % (mrc_full, mrc_full)
					os.system(cmd)
					os.system('rm %s~' % mrc_full)
				except:
					apDisplay.printMsg("IMOD function \'trimvol\' not found, trying to rotate with pyami. If the file is large and your RAM is small this might not end well...")
