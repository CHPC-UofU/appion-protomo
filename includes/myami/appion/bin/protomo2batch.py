...
def parseOptions():
	...
	parser.add_option('--runname', dest='runname',
		help= 'Name of protomorun directory as made by Appion, e.g. --runname=batchrun1')
	...
	parser.add_option('--tiltseriesranges', dest='tiltseriesranges', default="0",
		help= 'Range of tilt-series numbers to be processed. Should be in a comma/hyphen-separated list, e.g. 3,4,6-10')
	parser.add_option("--pixelsize_multiplier", dest="pixelsize_multiplier",
		help='Multiplies the pixelsize by this value. This is used only in the PHP, not in this file, e.g. --pixelsize_multiplier=2')
	...
	parser.add_option("--exporting_conversion", dest="exporting_conversion",  default="False",
		help="Tilt-series exporting and converting to IMOD/EMAN2, e.g. --exporting_conversion=True")
	parser.add_option("--runname_list", dest="runname_list",
		help="Comma-separated list of tilt-series alignment runnames to be exported. The best iteration will be chosen by default, e.g. --runname_list=tiltseries0004_thick400,tiltseries0071_thick750,tiltseries0099_thick500")
	parser.add_option("--frame_dir", dest="frame_dir",
		help="Directory where the frames are stored, e.g. --frame_dir=/gpfs/frames/me/21aug12a/rawdata/")
	parser.add_option("--frame_ext", dest="frame_ext",
		help="Full frame extention, including the dot '.' and any other suffixed dot, e.g. --frame_ext=.frames.tif")
	...
	parser.add_option("--psi", dest="psi", default=0.0,
		help='Psi stage orientation angle, e.g. --psi="5.4"')
	parser.add_option("--theta", dest="theta", default=0.0,
		help='Theta stage orientation angle, e.g. --theta="20.1"')
	parser.add_option("--phi", dest="phi", default=0.0,
		help='Phi stage orientation angle, e.g. --phi="-5.4"')
	...
	parser.add_option("--re_center_images", dest="re_center_images",  default="True",
		help="Re-center all images with respect to the alignment. This will increase the maximum allowed search area, which is useful in most cases, e.g. --re_center_images=True")
...
def protomoPrep(log_file, tiltseriesnumber, prep_options):
	...
	apProTomo2Prep.addOrientationAngles(tiltfilename_full, prep_options.psi, prep_options.theta, prep_options.phi)
...
def protomoCoarseAlign(log_file, tiltseriesnumber, coarse_options):
	...
	try:
		...
		if coarse_options.re_center_images == "True":
			apProTomo2Aligner.centerAlignment(tiltfile, coarse_options.dimx, coarse_options.dimy)
...
def protomoReconstruct(log_file, tiltseriesnumber, recon_options):
	...
	# Normalize
	try:
		command="newstack -meansd 0,1 %s %s" % (mrc_full, mrcn_full)
		proc=subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
		(code, err) = proc.communicate()
	except:
		pass
	#Switched from e2proc3d.py to newstack
...
def protomoExport(log_file, tiltseriesnumber, export_options):
	"""
	Creates tilt-series directory, copies raw images, creates series*.tlt file.
	Created tilt-series stack and SerialEM-formatted mdoc, which also works in Warp/M.
	...
	tilts, dose_list, accumulated_dose_list, new_ordered_imagelist, maxtilt, original_filename, defocus_list, pixelsize, magnification, original_ordered_mrc_files, num_frames_list = apProTomo2Prep.prepareTiltFile(export_options.sessionname, seriesname, tiltfilename_full, tiltseriesnumber, raw_path, export_options.frame_aligned, link=False, coarse="True", export=True)
	...
	for i in range(len(new_ordered_imagelist)):
		...
		try:
			image_dict['original_filename'] = export_options.frame_dir + os.path.basename(os.path.splitext(original_ordered_mrc_files[i])[0]) + export_options.frame_ext
		except:
			image_dict['original_filename'] = os.path.basename(original_ordered_mrc_files[i])
		image_dict['num_frames'] = num_frames_list[i]
		image_list.append(image_dict)
	sorted_image_list = sorted(image_list, key=lambda k: k['time'])
	
	mdoc = open(mdoc_file_full,'w')
	mdoc.write('PixelSpacing = %s\n' % export_options.pixelsize)
	mdoc.write('ImageFile = %s\n' % stack_file)
	mdoc.write('ImageSize = %s %s\n' % (export_options.dimx, export_options.dimy))
	mdoc.write('DataMode = 1\n')
	mdoc.write('\n')
	mdoc.write('[T = SerialEM-formatted Leginon data formatted for easy import to Appion-Protomo                %s]\n' % time.strftime("%d-%b-%y  %H:%M:%S", time.localtime(int(current_time))))
	mdoc.write('\n')
	try:
		rotated_tilt_azimuth = float(export_options.tilt_azimuth) + 90
		mdoc.write('[T =     Tilt axis angle = %s, binning = 1  spot = 8  camera = 1]\n' % export_options.tilt_azimuth)
	except:
		rotated_tilt_azimuth = 90
		mdoc.write('[T =     Tilt axis angle = 0, binning = 1  spot = 8  camera = 1]\n')
	mdoc.write('\n')
	
	# Many of these have dummy values because Warp seems to only require that they exist
	for data, i in zip(sorted_image_list, range(len(sorted_image_list))):
		mdoc.write('[ZValue = %s]\n' % i)
		mdoc.write('TiltAngle = %s\n' % data['tilt'])
		mdoc.write('StagePosition = 0 0\n')
		mdoc.write('StageZ = 0\n')
		mdoc.write('Magnification = %s\n' % magnification)
		mdoc.write('Intensity = 1\n')
		mdoc.write('ExposureDose = %s\n' % data['dose'])
		mdoc.write('PixelSpacing = %s\n' % export_options.pixelsize)
		mdoc.write('SpotSize = 1\n')
		defocus = data['defocus']*-1
		mdoc.write('Defocus = %s\n' % defocus)
		mdoc.write('ImageShift = 0 0\n')
		mdoc.write('RotationAngle = %s\n' % rotated_tilt_azimuth)
		mdoc.write('ExposureTime = 1\n')
		mdoc.write('Binning = 1\n')
		mdoc.write('CameraIndex = 1\n')
		mdoc.write('DividedBy2 = 1\n')
		mdoc.write('MagIndex = 30\n')
		mdoc.write('MinMaxMean = -1 1 0\n')
		mdoc.write('TargetDefocus = %s\n' % defocus)
		mdoc.write('SubFramePath = %s\n' % data['original_filename'])
		mdoc.write('NumSubFrames = %s\n' % data['num_frames'])
...
def protomoExportConvert(log_file, tiltseriesnumber, runname, exportconvert_options):
	"""
	Creates tilt-series directory, copies raw images, creates series*.tlt file.
	Created tilt-series stack and SerialEM-formatted mdoc, that also works in Warp/M.
	Cleans up temporary directories/files.
	"""
	export_dir = os.path.join(exportconvert_options.rundir, 'export')
	imod_export_dir = os.path.join(exportconvert_options.rundir, 'export', 'imod_export')
	imod_mdoc_dir = os.path.join(exportconvert_options.rundir, 'export', 'imod_export', 'mdocs')
	eman2_export_dir = os.path.join(exportconvert_options.rundir, 'export', 'eman2_export')
	eman2_tiltseries_dir = os.path.join(exportconvert_options.rundir, 'export', 'eman2_export', 'tiltseries')
	eman2_info_dir = os.path.join(exportconvert_options.rundir, 'export', 'eman2_export', 'info')
	os.system("mkdir -p %s %s %s 2>/dev/null" % (imod_mdoc_dir, eman2_tiltseries_dir, eman2_info_dir))

	it = int(glob.glob(os.path.join(exportconvert_options.rundir, runname, 'best.*'))[0].split('.')[1])
	ite = '%03d' % (it-1)
	itte = '%03d' % it
	seriesnumber = '%04d' % tiltseriesnumber
	seriesname = 'series' + seriesnumber + ite
	tiltfilename = seriesname + '.tlt'
	tiltfilename_full = os.path.join(exportconvert_options.rundir, runname, tiltfilename)
	outdir = os.path.join(export_dir, runname)
	raw_path = os.path.join(outdir, 'raw')
	basename = exportconvert_options.sessionname + '_' + runname.lstrip('tilt') + '_ite' + itte
	
	cmd = "awk '/AZIMUTH /{print $3}' %s" % (tiltfilename_full)
	...
	apDisplay.printMsg('Gathering Information for Tilt-Series #%s, Iteration #%s (%s)...' % (tiltseriesnumber, it, runname))
	f.write('Gathering Information for Tilt-Series #%s, Iteration #%s (%s)...' % (tiltseriesnumber, it, runname))
	
	tilts, dose_list, accumulated_dose_list, new_ordered_imagelist, maxtilt, original_filename, defocus_list, pixelsize, magnification, original_ordered_mrc_files, num_frames_list = apProTomo2Prep.prepareTiltFile(exportconvert_options.sessionname, seriesname, tiltfilename_full, tiltseriesnumber, raw_path, exportconvert_options.frame_aligned, link=False, coarse="True", export=True)
	
	stack_file = basename + '_' + original_filename + '.mrc'
	stack_file_full = os.path.join(export_dir, stack_file)
	mdoc_file_full = export_dir + '/' + basename + '_' + original_filename + '.mrc.mdoc'
	tlt_file_full = export_dir + '/' + basename + '_' + original_filename + '.mrc.tlt'
	xf_file_full = export_dir + '/' + basename + '_' + original_filename + '.xf'
	
	#Get all required SerialEM-formated information in a nice list of dictionaries
	current_time=time.time()

	files_list_protomo = apProTomo2Aligner.protomoToImodAndEman(tiltfilename_full, outdir, basename, pixelsize, exportconvert_options.dimx, exportconvert_options.dimy, tilt_azimuth)
	
	image_list = []
	for i in range(len(new_ordered_imagelist)):
		filename = os.path.basename(new_ordered_imagelist[i]).split('.')[0]
		if filename in files_list_protomo:
			image_dict={}
			image_dict['tilt'] = tilts[i]
			image_dict['dose'] = dose_list[i]
			image_dict['image'] = new_ordered_imagelist[i]
			...
			image_dict['accumulated_dose'] = accumulated_dose_list[i]
			image_dict['time'] = int(current_time + 10*image_dict['accumulated_dose'])
			image_dict['original_filename'] = exportconvert_options.frame_dir + os.path.basename(os.path.splitext(original_ordered_mrc_files[i])[0]) + exportconvert_options.frame_ext
			image_dict['num_frames'] = num_frames_list[i]
			image_list.append(image_dict)
	sorted_image_list = sorted(image_list, key=lambda k: k['time'])
	...
	tlt_angles_mdoc=[]
	mdoc = open(mdoc_file_full,'w')
	mdoc.write('PixelSpacing = %s\n' % exportconvert_options.pixelsize)
	mdoc.write('ImageFile = %s\n' % stack_file)
	mdoc.write('ImageSize = %s %s\n' % (exportconvert_options.dimx, exportconvert_options.dimy))
	mdoc.write('DataMode = 1\n')
	mdoc.write('\n')
	mdoc.write('[T = SerialEM-formatted Leginon data formatted for easy import to Appion-Protomo                %s]\n' % time.strftime("%d-%b-%y  %H:%M:%S", time.localtime(int(current_time))))
	mdoc.write('\n')
	rotated_tilt_azimuth = float(tilt_azimuth) + 90
	mdoc.write('[T =     Tilt axis angle = %s, binning = 1  spot = 8  camera = 1]\n' % tilt_azimuth)
	mdoc.write('\n')
	...
	for data, i in zip(sorted_image_list, range(len(sorted_image_list))):
		mdoc.write('[ZValue = %s]\n' % i)
		mdoc.write('TiltAngle = %s\n' % data['tilt'])
		tlt_angles_mdoc.append(data['tilt'])
		mdoc.write('StagePosition = 0 0\n')
		mdoc.write('StageZ = 0\n')
		mdoc.write('Magnification = %s\n' % magnification)
		mdoc.write('Intensity = 1\n')
	...
	# Link files needed by different softwares
	#os.system('ln %s/info/*.json %s/' % (outdir, eman2_info_dir))
	#os.system('ln %s/tiltseries/*.mrc %s/' % (outdir, eman2_tiltseries_dir))
	#os.system('ln %s/tiltseries/*.mrc %s' % (outdir, stack_file_full))
	#os.system('ln %s/*.tlt %s' % (outdir, tlt_file_full))
	#os.system('ln %s %s/' % (tlt_file_full, imod_export_dir))
	#os.system('ln %s/*.xf %s' % (outdir, xf_file_full))
	#os.system('ln %s %s/' % (xf_file_full, imod_export_dir))
	#os.system('ln %s %s/' % (mdoc_file_full, imod_mdoc_dir))
	
	os.system('cp %s/info/*.json %s/' % (outdir, eman2_info_dir))
	os.system('cp %s/tiltseries/*.mrc %s/' % (outdir, eman2_tiltseries_dir))
	os.system('cp %s/tiltseries/*.mrc %s' % (outdir, stack_file_full))
	os.system('cp %s/*.tlt %s' % (outdir, tlt_file_full))
	os.system('cp %s %s/' % (tlt_file_full, imod_export_dir))
	os.system('cp %s/*.xf %s' % (outdir, xf_file_full))
	os.system('cp %s %s/' % (xf_file_full, imod_export_dir))
	os.system('cp %s %s/' % (mdoc_file_full, imod_mdoc_dir))
...
if __name__ == '__main__':
	...
	if (options.exporting == "True"):
		...
		[p.join() for p in mp.active_children()]
		print "\n"
		apDisplay.printMsg("Exported tilt-series stacks can be found in:")
		apDisplay.printMsg("%s/export/" % options.rundir)
		print "\n"
		log = open(log_file,'a')


	#Tilt-series exporting & converting to IMOD & EMAN2 alignment files.
	if (options.exporting_conversion == "True"):
		apDisplay.printMsg("Appion-Protomo Tilt-Series Exporting Conversion Mode")
		apDisplay.printMsg("Tilt-series will be exported as a single tilt image stack, a SerialEM-formatted mdoc file, an IMOD xf file, and an EMAN2 json file.")
		apDisplay.printMsg("The best tilt-series alignment iteration for conversion to IMOD/EMAN2 will automatically be chosen.")
		apDisplay.printMsg("The IMOD xf file + mdoc should be sufficient for importing into most post-processing software, including Warp/M, emClarity, Relion, and novaCTF.")
		apDisplay.printMsg("The exported files should be ready for Warp/M import. You may need to change the frame path to be correct for Windows.")
		apDisplay.printMsg("The exported tilt-series are also formatted properly for import into Appion-Protomo (native and Docker).")
		log.write("Appion-Protomo Tilt-Series Exporting Mode\n")
		log.write("Tilt-series will be exported as a single tilt image stack, a SerialEM-formatted mdoc file, an IMOD xf file, and an EMAN2 json file.\n")
		log.write("The best tilt-series alignment iteration for conversion to IMOD/EMAN2 will automatically be chosen.\n")
		log.write("The IMOD xf file + mdoc should be sufficient for importing into most post-processing software, including Warp/M, emClarity, Relion, and novaCTF.\n")
		log.write("The exported files should be ready for Warp/M import. You may need to change the frame path to be correct for Windows.\n")
		log.write("The exported tilt-series are formatted properly for import into Appion-Protomo (native and Docker).\n")
		if (options.procs > 8):
			procs=8
		else:
		...
		runname_list=options.runname_list.rstrip(',').split(',')
		
		for runname, j in zip(runname_list, range(len(runname_list))):
			match = re.match(r"([a-z]+)([0-9]+)", runname.split('_')[0], re.I)
			if match:
				items = match.groups()
			tiltseriesnumber = int(items[1].lstrip("0"))
			p = mp.Process(target=protomoExportConvert, args=(log_file, tiltseriesnumber, runname, options,))
			p.start()
		...
		[p.join() for p in mp.active_children()]
		print "\n"
		apDisplay.printMsg("Exported tilt-series stacks, mdocs, xf files, and json files can be found in:")
		apDisplay.printMsg("%s/export/" % options.rundir)
		print "\n"
		log = open(log_file,'a')
	...
	log.write('If so, kindly post your issue on the following Appion-Protomo Google group or email Alex at anoble@nysbc.org explaining the issue and include this log file.\n')
	log.write('https://groups.google.com/forum/#!topic/appion-protomo\n')
