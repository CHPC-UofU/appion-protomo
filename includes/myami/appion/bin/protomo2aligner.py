...
# Hack from Anchi.  For unknown reason, queries can not
# be run without first importing apDatabase and using it for something.
from appionlib import apDatabase
apDatabase.getSessionDataFromSessionName('20nov01a')
...
class ProTomo2Aligner(basicScript.BasicScript):
	...
	def setupParserOptions(self):
		...
		self.parser.add_option('--shift_limit', dest='shift_limit', default=90, type='float', metavar='float', help='Percentage of image size, above which shifts with higher shifts will be discarded for all images, e.g. --shift_limit=50') 
		
		self.parser.add_option('--angle_limit', dest='angle_limit', default=90, type='float', metavar='float', help='Only remove images from the tilt file greater than abs(angle_limit), e.g. --angle_limit=35') 
		...
		self.parser.add_option("--r1_cmdiameter", dest="r1_cmdiameter",
			help="Size of region for center of mass calculation switch, e.g. --r1_cmdiameter=19.0",)
		...
		self.parser.add_option("--psi", dest="psi", default=0.0,
			help='Psi stage orientation angle, e.g. --psi="5.4"')

		self.parser.add_option("--theta", dest="theta", default=0.0,
			help='Theta stage orientation angle, e.g. --theta="20.1"')

		self.parser.add_option("--phi", dest="phi", default=0.0,
		...
		self.parser.add_option("--re_center_images", dest="re_center_images",  default="False",
		...
		self.parser.add_option("--change_refimg", dest="change_refimg",  default="True",
	...
	def start(self):
		...
		if (self.params['coarse'] == 'True' and self.params['my_tlt'] == 'False'):
			...
			apProTomo2Aligner.reOrderImagesInTltFile(tiltfilename_full)
			...
			apProTomo2Prep.addOrientationAngles(tiltfilename_full, self.params['psi'], self.params['theta'], self.params['phi'])
		...
		if self.params['coarse'] == 'True':
			...
			if self.params['re_center_images'] == "True":
			...
			if self.params['re_center_images'] == "True":
				apProTomo2Aligner.centerAlignment(imod_coarse_tlt_file, self.params['dimx'], self.params['dimy'])
			apProTomo2Aligner.findMaxSearchArea(imod_coarse_tlt_file, self.params['dimx'], self.params['dimy'])
		...
		f.write('If so, kindly post your issue on the following Appion-Protomo Google group or email Alex at anoble@nysbc.org explaining the issue and include this log file.\n')
		f.write('https://groups.google.com/forum/#!topic/appion-protomo\n')
